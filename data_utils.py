...
def load_dataset(dataset_name: str,
                 ...
                 manual_dir: Optional[str] = None):  # 新增参数:
  ...
  # 构造 download_and_prepare_kwargs 参数（6.4加）
  download_and_prepare_kwargs = {}
  if manual_dir is not None:
      # 确保 manual_dir 是字符串类型
      manual_dir_str = str(manual_dir)
      download_config = tfds.download.DownloadConfig(manual_dir=manual_dir_str)
  ...
  if dataset_name.startswith('srn'):
    ...
    ds = ds.map(process_celeba, num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def process_celeba(batch: Batch):
  ...
  image = tf.image.resize(image, [128, 128])  # [64, 64];[64, 64, 3]
  return {'array': image}
